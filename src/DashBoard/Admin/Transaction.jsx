...
const Transaction = () => {
  const { data: weeklyData, isPending: isWeeklyPending, isError: isWeeklyError, error: weeklyError } = useQuery({ queryKey: "weeklyData", queryFn: () => fetchTransactionData("get/weak") });
  const { data: monthlyData, isPending: isMonthlyPending, isError: isMonthlyError, error: monthlyError } = useQuery({ queryKey: "monthlyData", queryFn: () => fetchTransactionData("get/month") });                    

  if (isWeeklyPending || isMonthlyPending) {
    return <>loading....................</>;
  }

  if (isWeeklyError || isMonthlyError) {
    return <span>Error: {weeklyError?.message || monthlyError.message}</span>;
  }
